...
#binomial.glm1 <- glm(f1, data = brv.train, family = binomial())  #Logisitic Regression


#naiveBayes.m1 <- naiveBayes(f1, data=brv.train)
#pre1 <- predictThresh(predict(naiveBayes.m1, brv.train, type = "raw"), K=4)
#print(ZeroOneLoss(pre1, brv.train$clicked))
...
roc <- naivebayesROC()
roc.plot <- ggplot(roc, aes(fpr, tpr)) + geom_point() + xlim(0,1) + ylim(0,1) +geom_abline(slope=1, intercept=0, color="blue")
print(roc.plot)

#svm.m1 <- svm(f1, data=brv.train, class.weights = c("0"=1, "1"=4))
#pre2 <- predict(svm.m1, brv.train)
#print(ZeroOneLoss(pre2, brv.train$clicked))
...
svmROC <- function() {
  ...
  for(i in seq(0,0.5,0.01)){
    ...
    cm <- ConfusionMatrix(y_pred = pre1, y_true = brv.train$clicked)
    print(cm)
    tn <- cm[1]
    fp <- cm[2]
    fn <- cm[3]
